...
const {
  dirname
      ...
      {
  EventEmitter
} = __webpack_require__(614); // Защищенные поля для PoonyaPrototype
...
class CodeEmitter extends EventEmitter {
  ...
  constructor(input, import_s = [], logger = console, onload) {
    ...
    if (typeof input === "string") {
      ...
      _.path = normalize(typeof input.path === "string" ? ['', '.'].includes(extname(input.path)) ? input.path + '.po' : input.path : module.parent ? module.parent.filename : module.filename); // Защищаю от выполнения браузерного кода в nodejs
